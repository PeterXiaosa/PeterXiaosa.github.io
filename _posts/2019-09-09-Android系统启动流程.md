...
### Launcher的启动
Launcher的启动由AMS(ActivityManagerService)启动之后开始的，通过一系列的调用`AMS(systemReady())-ActivityStackSupervisor(resumeFocusedStackTopActivityLocked())-ActivityStack(resumeTopActivityUncheckedLocked())-ActivityStack(resumeTopActivityInnerLocked())-ActivityStackSupervisor(resumeHomeStackTask)-AMS(startHomeActivityLocked())`一系列的调用，最后调用AMS的`startHomeActivityLocked()`方法。
...
* 遗留问题：https://www.zhihu.com/question/345196352